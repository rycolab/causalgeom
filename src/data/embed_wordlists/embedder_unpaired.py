...
def get_emb_outfile_paths(concept, model_name):
    l0_emb_outfile = os.path.join(DATASETS, f"processed/CEBaB/embedded_word_lists/{concept}/{model_name}_l0_embeds.npy")
    l1_emb_outfile = os.path.join(DATASETS, f"processed/CEBaB/embedded_word_lists/{concept}/{model_name}_l1_embeds.npy")
    return l0_emb_outfile, l1_emb_outfile

def get_token_list_outfile_paths(concept, model_name, single_token):
    
    l0_outfile = os.path.join(DATASETS, f"processed/CEBaB/tokenized_lists/{concept}/{model_name}_l0_token_list.npy")
    l1_outfile = os.path.join(DATASETS, f"processed/CEBaB/tokenized_lists/{concept}/{model_name}_l1_token_list.npy")

    if not single_token:
        l0_outfile = l0_outfile[:-4] + "_all_lengths.npy"
        l1_outfile = l1_outfile[:-4] + "_all_lengths.npy"
    return l0_outfile, l1_outfile

def load_concept_token_lists(concept, model_name, single_token):
    l0_tl_file, l1_tl_file = get_token_list_outfile_paths(
...
#TODO: delete, not relevant to unpaired scenario
...
def get_unique_lemma_lists(lemma_list_path, model_name, tokenizer, add_space, single_token):
    ...
    if single_token:
        ...
        # check if any words are tokenized weirdly
...
if __name__=="__main__":
    ...
    lemmalist_path = get_wordlist_paths(concept)
    l0_emb_outfile, l1_emb_outfile = get_emb_outfile_paths(
        concept, model_name)
    l0_tl_outfile, l1_tl_outfile = get_token_list_outfile_paths(
        concept, model_name, single_token)
    ...
    l0_tok, l1_tok = get_unique_lemma_lists(

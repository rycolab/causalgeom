...
def prep_run_data_from_test_only(X, facts, foils, y, train_share=0.5):
...
def intervene_test_set(test_hs, case, l0_dev_hs, l1_dev_hs, all_hs, 
    V, l0_tl, l1_tl, P, I_P, inner_nsamples, nucleus=False):
    ...
    for h, faid, foid in tqdm(test_hs):
        ...
        inner_qxhs = compute_inner_loop_qxhs(
            "hpar", h, all_hs, P, I_P, V, inner_nsamples, processor=processor
        )
        ...
        # do intervention
        #TODO: the intervention is not using the right computation
        #h_erase = h.T @ I_P
        #l0_avgp_probs = compute_avg_int(h_erase, l0_dev_hs, inner_nsamples, V, processor)
        #l1_avgp_probs = compute_avg_int(h_erase, l1_dev_hs, inner_nsamples, V, processor)
        l0_avgp_probs_all = compute_inner_loop_qxhs(
            "hpar", h, l0_dev_hs, P, I_P, V, inner_nsamples, processor=processor
        )
        l0_avgp_probs_fix = l0_avgp_probs_all.mean(axis=0)
        l1_avgp_probs_all = compute_inner_loop_qxhs(
            "hpar", h, l1_dev_hs, P, I_P, V, inner_nsamples, processor=processor
        )
        l1_avgp_probs_fix = l1_avgp_probs_all.mean(axis=0)
        ...
        #)
        score = score_post_int(
            base_distrib, I_P_distrib, l0_avgp_probs_fix, l1_avgp_probs_fix, 
            faid, foid, case, l0_tl, l1_tl
        )
        scores.append(score)
    return scores
...
def compute_int_eval_run(model_name, concept, run, run_path, 
    ...
    #)
    train_l0_hs, train_l1_hs = filter_all_hs_w_ys(
        run["X_val"], run["facts_val"], run["foils_val"], run["y_val"]
    )
    test_l0_hs, test_l1_hs = filter_all_hs_w_ys(
        run["X_test"], run["facts_test"], run["foils_test"], run["y_test"]
    )

    _, _, _, all_hs = prep_generated_data(model_name, nucleus)
    ...
    scores_l0 = intervene_test_set(
        ...
        V, l0_tl, l1_tl, P, I_P, nsamples_dev, nucleus=False
    )
...
def compute_int_eval_folder(model_name, concept, run_output_folder, 
    nsamples_dev, nsamples_test, nucleus, output_folder, iteration):
    ...
    for run_file in run_files:
        ...
        if run["config"]["k"] != 1:
            ...
            int_accs_df = compute_int_eval_run(
                ...
                nucleus=nucleus, iteration=iteration
            )
...
if __name__=="__main__":
    ...
    logging.info(
        ...
        f"{model_name} with nucleus {nucleus} "
        f"from folders: {run_output_folders}"
    )    
    ...
    for folder in run_output_folders:
        for i in range(nruns):
            compute_int_eval_folder(
                ...
                nsamples_dev, nsamples_test, nucleus,
                output_folder, i
            )

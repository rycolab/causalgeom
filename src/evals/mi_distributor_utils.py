...
# Arg Handling                         #
#########################################
def get_nucleus_arg(data_source):
    """ Use samples generated by model with nucleus sampling 
    for natural text (test set) samples.
    """
    if data_source in ["gen_nucleus_concept", "gen_nucleus_all"]:
        nucleus = True
    elif data_source in ["gen_ancestral_concept", "gen_ancestral_all"]:
        nucleus = False
    elif data_source in ["natural_all", "natural_concept"]:
        #TODO: THIS SHOULDNT BE IN HERE ITS A SHORTCUT FOR CEBAB
        nucleus = False 
    else:
        raise ValueError(f"Incorrect {data_source} argument")
    return nucleus

#########################################
# Eval Directory Handling               #
#########################################
def get_run_path_info(run_path):
    run_path_parts = Path(run_path).parts
    run_output_dir = run_path_parts[-5]
...
def get_cxt_toks(model_name, data_source, l0_gens, l1_gens, other_gens, 
    max_n_cxts, cxt_max_length_pct=1):
    """ Process and filters generated context strings
    """
    l0_cxt_toks = [all_tok for _,_,_,all_tok in l0_gens]
    l1_cxt_toks = [all_tok for _,_,_,all_tok in l1_gens]
    ...
    del l0_cxt_toks
    del l1_cxt_toks
    if data_source in ["gen_ancestral_concept", "gen_nucleus_concept"]:
        concept_cxt_toks = sample_cxt_toks(concept_cxt_toks, max_n_cxts)
        return concept_cxt_toks
    elif data_source in ["gen_ancestral_all", "gen_nucleus_all"]:
        ...
        raise NotImplementedError(
            ...
            " context tokens not implemented"
        )

def prep_generated_data(model_name, concept, nucleus, data_source, 
    torch_dtype, cxt_max_length_pct, max_n_cxts, max_n_all_hs):
    """ Loads generated text and outputs:
    - all_hs: generated hs by the model
    - cxt_toks: either concept or all context tokens, depending on 
                data_source arg
    ...
    if data_source in ["train_all", "train_concept", 
                       "test_all", "test_concept"]:
        cxt_toks = None
        len_cxt_toks = 0
    else:
        cxt_toks = get_cxt_toks(
            model_name, data_source, l0_gens, l1_gens, other_gens, 
            max_n_cxts, cxt_max_length_pct
        )
